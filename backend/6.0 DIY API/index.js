...
app.get("/filter", (req, res) => {
  const result = jokes.filter((joke) => joke.jokeType == req.query.type);
  res.json(result);
});
...
app.post("/jokes", (req, res) => {
  if (req.body.text && req.body.type) {
    const lastIndex = jokes[jokes.length - 1].id;
    jokes.push({
      id: lastIndex + 1,
...
app.put("/jokes/:id", (req, res) => {
  const joke = jokes.findIndex((joke) => joke.id == req.params.id);
  if (req.body.text && req.body.type) {
    jokes[joke] = {
      id: req.params.id,
      jokeText: req.body.text,
  ...
  if (req.body.text || req.body.type) {
    if (req.body.text) {
      jokes[joke] = {
        id: req.params.id,
        jokeText: req.body.text,
        jokeType: jokes[joke].jokeType,
      };
    } else {
      jokes[joke] = {
        id: req.params.id,
        jokeText: jokes[joke].jokeText,
        jokeType: req.body.type,
      };
    }
  }
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index >= 0) {
    ...
    res.status(404).json({
      error:
        "Joke with id: " + req.params.id + " not found. No jokes were deleted.",
    });
  }
});
